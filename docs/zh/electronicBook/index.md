## 最近在看的电子书（电子书均为电子书内标记处所获如想要看更多请自行联系书中对应联系方式）

## 1.面试宝典

[2023前端面试上岸手册（八股文）](../../assets/electronicBook/2023前端面试上岸手册（八股文）.pdf)


[2023前端面试上岸手册（八股文）](../../assets/electronicBook/前端面试通关宝典.pdf)

[P8大佬的算法解题笔记）](../../assets/electronicBook/P8大佬的算法解题笔记.pdf)

## 2.逆向工程

[程序员的数学(第2版)%20([日]结城浩)%20(Z-Library](../../assets/electronicBook/程序员的数学(第2版)%20([日]结城浩)%20(Z-Library).pdf)

[漫画算法：小灰的算法之旅.pdf](../../assets/electronicBook/漫画算法：小灰的算法之旅.pdf)

## 3.Python

[Python编程：从入门到实践](../../assets/electronicBook/Python编程：从入门到实践（第3版）.pdf)